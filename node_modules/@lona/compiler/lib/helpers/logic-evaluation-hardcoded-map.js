...
const logic_unify_1 = require("./logic-unify");
exports.hardcoded = {
    functionCallExpression: {
        'Color.saturate': (node, baseColor, percent) => {
            if (!baseColor ||
                ...
                throw new Error('The first argument of `Color.saturate` need to be a color');
            }
            const parsedColor = color_1.default(baseColor.memory.value.value.memory.value);
            if (!percent || percent.memory.type !== 'number') {
                throw new Error('The second argument of `Color.saturate` need to be a number');
            }
            return {
                type: logic_unify_1.color,
                memory: {
                    type: 'record',
                    value: {
                        value: {
                            type: logic_unify_1.string,
                            memory: {
                                type: 'string',
        ...
        'Color.setHue': (node, baseColor, hue) => {
            if (!baseColor ||
                ...
                !baseColor.memory.value.value ||
                baseColor.memory.value.value.memory.type !== 'string') {
                throw new Error('The first argument of `Color.setHue` need to be a color');
            }
            const parsedColor = color_1.default(baseColor.memory.value.value.memory.value);
            if (!hue || hue.memory.type !== 'number') {
                throw new Error('The second argument of `Color.setHue` need to be a number');
            }
            return {
                type: logic_unify_1.color,
                memory: {
        ...
        'Color.setSaturation': (node, baseColor, saturation) => {
            if (!baseColor ||
                ...
                !baseColor.memory.value.value ||
                baseColor.memory.value.value.memory.type !== 'string') {
                throw new Error('The first argument of `Color.setSaturation` need to be a color');
            }
            const parsedColor = color_1.default(baseColor.memory.value.value.memory.value);
            if (!saturation || saturation.memory.type !== 'number') {
                throw new Error('The second argument of `Color.setSaturation` need to be a number');
            }
            return {
                type: logic_unify_1.color,
                memory: {
        ...
        'Color.setLightness': (node, baseColor, lightness) => {
            if (!baseColor ||
                ...
                !baseColor.memory.value.value ||
                baseColor.memory.value.value.memory.type !== 'string') {
                throw new Error('The first argument of `Color.setLightness` need to be a color');
            }
            const parsedColor = color_1.default(baseColor.memory.value.value.memory.value);
            if (!lightness || lightness.memory.type !== 'number') {
                throw new Error('The second argument of `Color.setLightness` need to be a number');
            }
            return {
                type: logic_unify_1.color,
                memory: {
        ...
        'Color.fromHSL': (node, hue, saturation, lightness) => {
            if (!hue || hue.memory.type !== 'number') {
                throw new Error('The first argument of `Color.fromHSL` need to be a number');
            }
            if (!saturation || saturation.memory.type !== 'number') {
                throw new Error('The second argument of `Color.fromHSL` need to be a number');
            }
            if (!lightness || lightness.memory.type !== 'number') {
                throw new Error('The third argument of `Color.fromHSL` need to be a number');
            }
            const parsedColor = color_1.default({
                h: hue.memory.value,
        ...
        'Boolean.or': (node, a, b) => {
            if (!a || a.memory.type !== 'bool') {
                throw new Error('The first argument of `Boolean.or` need to be a boolean');
            }
            if (!b || b.memory.type !== 'bool') {
                throw new Error('The second argument of `Boolean.or` need to be a boolean');
            }
            return {
                type: logic_unify_1.bool,
                memory: {
                    type: 'bool',
        ...
        'Boolean.and': (node, a, b) => {
            if (!a || a.memory.type !== 'bool') {
                throw new Error('The first argument of `Boolean.and` need to be a boolean');
            }
            if (!b || b.memory.type !== 'bool') {
                throw new Error('The second argument of `Boolean.and` need to be a boolean');
            }
            return {
                type: logic_unify_1.bool,
                memory: {
        ...
        'String.concat': (node, a, b) => {
            if (!a || a.memory.type !== 'string') {
                throw new Error('The first argument of `String.concat` need to be a string');
            }
            if (!b || b.memory.type !== 'string') {
                throw new Error('The second argument of `String.concat` need to be a string');
            }
            return {
                type: logic_unify_1.string,
                memory: {
        ...
        'Number.range': (node, from, to, by) => {
            if (!from || from.memory.type !== 'number') {
                throw new Error('The first argument of `Number.range` need to be a number');
            }
            if (!to || to.memory.type !== 'number') {
                throw new Error('The second argument of `Number.range` need to be a number');
            }
            if (!by || by.memory.type !== 'number') {
                throw new Error('The third argument of `Number.range` need to be a number');
            }
            const arr = [];
            if (by.memory.value === 0 || by.memory.value === -0) {
                ...
                for (let i = from.memory.value; i < to.memory.value; i += by.memory.value) {
                    arr.push(i);
                }
            }
            return {
                type: logic_unify_1.array(logic_unify_1.number),
                memory: {
                    type: 'array',
                    value: arr.map(x => ({
                        type: logic_unify_1.number,
                        memory: {
                            type: 'number',
                            value: x,
                        },
