...
function isVariableDeclaration(x) {
...
const declaration = (node, context) => {
    ...
    switch (node.type) {
        ...
        case 'namespace': {
            const newContext = Object.assign(Object.assign({}, context), { isStatic: true });
            return {
        ...
        case 'record': {
            const newContext = Object.assign(Object.assign({}, context), { isStatic: false });
            const memberVariables = node.data.declarations.filter(isVariableDeclaration);
            const initFunction = {
            ...
            return {
                ...
                data: {
                    ...
                    body: (memberVariables.length
                        ? [initFunction]
                        : []).concat(memberVariables.map(x => declaration({ type: 'variable', data: Object.assign({}, x.data) }, newContext))),
                },
            };
        }
        case 'enumeration': {
            return {
                ...
                data: {
                    ...
                    body: node.data.cases
                        ...
                        const associatedValueTypes = x.data.associatedValueTypes.filter(y => y.type !== 'placeholder');
                        const associatedType = associatedValueTypes.length === 0
                            ? undefined
                            : {
                                type: 'TupleType',
                    ...
                    parameters: node.data.parameters
                        ...
                        return {
                            ...
                            data: {
                                ...
                                defaultValue: x.data.defaultValue && x.data.defaultValue.type !== 'none'
                        ...
                        .filter(utils_1.nonNullable),
                    result: node.data.returnType.type !== 'placeholder'
...
const convertNativeType = (typeName, _context) => {
    switch (typeName) {
        case 'Boolean':
            return 'Bool';
        case 'Number':
...
const typeAnnotation = (node, context) => {
    if (!node) {
        context.helpers.reporter.warn('no type annotation when needed remaining in file');
        ...
        case 'functionType': {
            return {
                ...
                data: {
                    ...
                    returnType: node.data.returnType.type !== 'placeholder'
                        ? typeAnnotation(node.data.returnType, context)
                        : undefined,
                },
            };
...
const genericParameter = (node, context) => {
    switch (node.type) {
        case 'parameter': {
            return {
                type: 'TypeName',
        ...
        case 'placeholder': {
            context.helpers.reporter.warn('Generic type placeholder remaining in file');
            return { type: 'TypeName', data: '_' };
        }
        default: {
            utils_1.typeNever(node, context.helpers.reporter.warn);
